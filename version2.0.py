...
SIDE_R_Z_ABS_MAX_MM = 700.0
# 450 / 500 / 450 的左右宽度 -> Z 分界在 ±250 mm
SIDE_R_Z_SPLIT_CENTER_MM = 250.0
SIDE_R_LEFT_AREA_ID = 6          # Z in [+250, +700]
SIDE_R_MID_AREA_ID  = 7          # Z in [-250, +250]
SIDE_R_RIGHT_AREA_ID= 8          # Z in [-700, -250]
...
def classify_area_for_cam(wx_mm, wy_mm, wz_mm, cam_idx):
    """
    cam_idx == 1 -> 正面(1/2) 2
    cam_idx == 0 -> 左侧(3/4/5) 0
    cam_idx == 2 -> 右侧(6/7/8) 1
    如果你的设备顺序不同，在这里改映射即可。
    """
    if cam_idx == 2:
                ...
                for *poly, conf, cls in reversed(det_numpy):
                    ...
                    if csv_writer is not None:
                        ...
                        if world_coords is not None:
                            ...
                            area_id_csv = classify_area_for_cam(wx_mm, wy_mm, wz_mm, device_index)
...
def run_sequential_multicams(weights=ROOT / 'runs/best.pt',
    ...
    for cam_idx in range(n_use):
        ...
        results.append(res)
        # 这行原来用了 res['got_valid_world']，现在改成统计每相机的行数
        LOGGER.info(
            f"========== 完成相机 Cam {cam_idx}：areas={sorted(list(res.get('lines_by_area', {}).keys()))} ==========\n")
        time.sleep(0.5)

    # 1) 聚合所有相机的区域行（只保留已检测到的区域；不写 NONE）
    agg_lines = []
    ...
    # 会话结束标记（保留）
    # try:
    #     udp_send_text("VISION_SESSION:END", udp_ip, udp_port, timeout=udp_timeout, wait_ack=False)
    #     LOGGER.info("[UDP] 已发送会话结束标记")
    # except Exception as e:
    #     LOGGER.warning(f"[UDP] 会话结束标记发送失败: {e}")
...
def run(weights=ROOT / 'runs/best.pt',
    ...
    return run_sequential_multicams(weights=weights, device=device, project=project, name=name, exist_ok=exist_ok,
                                    ...
                                    max_cams=1, max_wait_s=max_wait_s, break_after_first=break_after_first)
...
def parse_opt():
    ...
    # 可视化
    parser.add_argument('--pc-viz', action='store_true', default=False, help='Open3D可视化（阻塞）')
    ...
    # 几何阈值（mm）
    parser.add_argument('--pc-gate-z-max-mm', type=float, default=1400, help='Z<=此值(mm)')
    ...
    # UDP/外参/语音
    parser.add_argument('--udp-ip', type=str, default='192.168.1.219', help='UDP receiver IP')
