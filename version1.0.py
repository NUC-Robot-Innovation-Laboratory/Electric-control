...
#  YOLOv5 + Orbbec Depth 支持版 + 基于官方 AlignFilter+PointCloudFilter 的点云ROI拟合
#  相机坐标系X估计（圆柱中心），支持Open3D可视化与ΔX基线
#  已修复：彩色相机内参、点云单位（米）、PointCloudFilter 比例、xyz_map 兜底生成、像素投影等
#  @version 1.0
#  @file 完成了手眼标定 与坐标系转化  通过点云获得了准确的世界坐标系  来让stewart平台抓取
# ******************************************************************************
                    ...
                    for *poly, conf, cls in reversed(det_numpy):
                        ...
                        if csv_writer is not None:
                            ...
                            if cam_coords is not None:
                                ...
                                row += [None, None, None]
                            if world_coords is not None:
                                row += [(-1) * float(world_coords[0] * 1000), float(world_coords[1] * 1000),
